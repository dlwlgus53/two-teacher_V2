...
class VerifyData:
    ...
    def pos_aug_dst(self, dst, user, value_dict, seed, neg_nums):
        ...
        def replace(dst, value_dict, user):
            try:
    ...
    def seperate_data(self, dataset):
        ...
        for dial in dataset:
            ...
            for t_id, turn in enumerate(dial):
                ...
                for (pos_user, pos_dst), ans_type_ in self.pos_aug_dst(turn['belief'], turn['user'], value_dict, t_id, self.neg_nums): # here neg sampling
                    pos_turn_text = self.make_dial(turn_text, pos_user)
...
if __name__ == '__main__':
    ...
    dataset = VerifyData(tokenizer, args.labeled_data_path,\
        'train', log_folder, args.neg_nums, short=1, upsamp=True)
